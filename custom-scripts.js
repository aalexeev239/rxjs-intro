(function (rxjs) {
    ...
    ***************************************************************************** */
    ...
    function renderNext(value, timelineElement) {
        ...
        const last$$1 = timelineElement.lastElementChild;

        if (last$$1) {
            const lastOffset = parseInt(last$$1.style.left, 10);
    ...
    function renderCompleteElement(timelineElement) {
        ...
        if (last$$1) {
            ...
            if (offset - lastOffset < last$$1.offsetWidth) {
    ...
    function startStream(parentElement, input$, delayTime = 0) {
        const timelineElement = setTimeline(parentElement);

        const subscription = rxjs.of(null)
            .pipe(
                delay(delayTime),
                switchMap(() => input$),
                takeUntil(
    ...
    function fetchData(url) {
        ...
        return rxjs.of(result)
            .pipe(
                delay(300 + Math.floor(Math.random() * 1000))
            );
    }
